...
class EmployeeInformationViewSet(CustomViewSet):
    ...
    queryset = EmployeeInformation.objects.exclude(
        user__user_informations__user_type__name__in =['Shop User', 'Shop']
        )
    lookup_field = 'slug'
    ...
    @log_activity
    def sales_wise_employee_list(self, request, *args, **kwargs):
        order_item_qs = OrderItem.objects.all()
        
        qs = EmployeeInformation.objects.filter(user__is_active = True, user__in = order_item_qs.values_list('created_by', flat = True)).exclude(user__user_informations__user_type__name='Shop')
        
        qs = self.filter_queryset(qs)
        
        page_qs = self.paginate_queryset(qs)
        serializer = EmployeeInformationListSerializer(instance=page_qs, many=True)
    ...
    @log_activity
    def sales_wise_employee_details(self, request, slug, *args, **kwargs):
        ...
        serializer = EmployeeSalesDetailsSerializer(employee_qs)
    ...
    def create(self, request, user_id, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if pos_area:
                pos_area_qs = POSArea.objects.filter(slug = pos_area).last()
                if not pos_area_qs:
...
# class EmployeeGuardianInformationViewSet(CustomViewSet):
#     queryset = EmployeeGuardianInformation.objects.all()
#     serializer_class = EmployeeGuardianInformationSerializer
#     permission_classes = [CheckCustomPermission]
#     lookup_field = 'pk'
...
#                     serializer = serializer_class(data=item_data)
#                     if serializer.is_valid():
#                         guardian_info_instance = serializer.save(created_by=self.request.user)              
#                         employee_param = self.kwargs.get('employee_param')
#                         try:                       
#                             employee_id = int(employee_param)
#                             employee_info_instance = get_object_or_404(EmployeeInformation, id=employee_id)
#                         except ValueError:
#                             employee_info_instance = get_object_or_404(EmployeeInformation, slug=employee_param)
#                         employee_info_instance.guardian_information.add(guardian_info_instance)
#                         results.append({
...
#         return ResponseWrapper(data=results, msg='created', status=200)

#     def retriveEmployee(self, request, *args, **kwargs):
...
#         queryset = employee_info_instance.guardian_information.all()
...
class EmployeeDownloadViewSet(CustomViewSet):
    queryset = EmployeeInformation.objects.all().order_by("?")
    lookup_field = 'pk'
    serializer_class = EmployeeLisDownloadSerializer
    permission_classes = [CheckCustomPermission]
    filter_backends = (
        DjangoFilterBackend,
        filters.OrderingFilter,
    ...
    def list(self, request, *args, **kwargs):
        ...
        serializer = EmployeeLisDownloadSerializer(queryset, many=True)

        data = Dataset(headers=['SI', 'Employee Id', 'Name', 'NID'])
        
        index = 1
        for entry in serializer.data:
            print(f"Row Number = {index}, Name = {entry['name']}")
        ...
        response['Content-Disposition'] = f'attachment; filename="Employee - {today_date}.xlsx"'
        return response
