...
from location.serializers import AreaSerializer, OfficeLocationListSerializer
from user.models import UserInformation
from user.serializers import BaseSerializer, UserSerializer
...
class EmployeeSalesDetailsSerializer(serializers.ModelSerializer):
    user = UserInformationBaseSerializer(read_only = True)
    designations = EmployeeDesignationSerializer(read_only=True)
    work_station = OfficeLocationListSerializer(read_only=True)
    joining_date = serializers.DateField(format="%m/%d/%Y")
    date_of_birth = serializers.DateField(format="%m/%d/%Y")
    sales_report = serializers.SerializerMethodField(read_only = True)
    image = serializers.SerializerMethodField(read_only = True)
    ...
    def get_image(self, obj):
        image = 'https://gprmain.sgp1.cdn.digitaloceanspaces.com/dev-gprojukti/test/8292bf8228d947f29d6ef7a6b55de87e-CategoryImage.jpg'
        if obj.image:
            image = obj.image
    ...
    def get_sales_report(self, obj):
        ...
        corporate_sell_list = []
        gift_sell_list = []
        pc_builder_sell_list = []
        
        order_type_list = ORDER_TYPE
        order_status_list = ORDER_STATUS
        
        sell_list = {
            'total_order_received' : 43,
            'total_order_confirm' : 33,
            'total_order_preliminary_qc' : 53,
            'total_order_detailed_qc' : 63,
            'total_order_packaged' : 33,
            'total_order_ready_for_pickup' : 23,
            'total_order_dispatch' : 13,
            'total_order_cancelled' : 54,
            'total_order_return' : 77,
            'total_order_delivered' : 73,
            'total_order_gsheba_return' : 30,
        }
        
        ecommerce_sell_list.append(sell_list)
        e_retail_sell_list.append(sell_list)
        pos_sell_list.append(sell_list)
        on_the_go_sell_list.append(sell_list)
        b2b_sell_list.append(sell_list)
        gift_sell_list.append(sell_list)
        pc_builder_sell_list.append(sell_list)
        
        context = {
            'ecommerce_sell_list': ecommerce_sell_list,
            'e_retail_sell_list': e_retail_sell_list,
            'pos_sell_list': pos_sell_list,
            'on_the_go_sell_list': on_the_go_sell_list,
            'b2b_sell_list': b2b_sell_list,
            'corporate_sell_list': corporate_sell_list,
            'gift_sell_list': gift_sell_list,
            'pc_builder_sell_list': pc_builder_sell_list,
        }
        
        return context
    
    
class EmployeeLisDownloadSerializer(serializers.ModelSerializer):
    class Meta:
        model = EmployeeInformation
        fields = [
            'id',
